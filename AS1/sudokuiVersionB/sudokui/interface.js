...
function encodeboardstate(state) {
    ...
    if ('answer' in state) {
        result.answer = encodepuzzle(state.answer);
    }
    // work: a list of N^2 bitmasks from 0-(2^N-1) for small numbers.
    if ('work' in state) {
        result.work = arraytobase64(state.work);
    }
    // seed: the puzzle number; 1 is the first page puzzle.
    if ('seed' in state) {
        result.seed = state.seed;
    }
    if('totalClear' in state){
        result.totalClear = state.totalClear;
    }
    if ('hintCount' in state) {
        result.hintCount = state.hintCount;
    }

    if('usefulHintCount' in state){
        result.usefulHintCount = state.usefulHintCount;
    }

    if('failedSubmits' in state) {
        result.failedSubmits = state.failedSubmits;
    }
    if('erased' in state) {
        result.erased = state.erased;
    }
    // gentime: when the puzzle was created, unix time milliseconds.
    if ('gentime' in state) {
        result.gentime = state.gentime;
    }
...
function decodeboardstate(data) {
    ...
    if ('hintCount' in data) {
        result.hintCount = data.hintCount;
    }
    if ('usefulHintCount' in data){
        result.usefulHintCount = data.usefulHintCount;
    }
    if ('totalClear' in data){
        result.totalClear = data.totalClear;
    }
    if ('failedSubmits' in data){
        result.failedSubmits = data.failedSubmits;
    }
    if('erased' in data){
        result.erased = data.erased;
    }
    if ('gentime' in data) {
        result.gentime = data.gentime;
    }
    if ('elapsed' in data) {
...
$(document).on('mousedown', 'td.sudoku-cell', function (ev) {
    ...
    if (num === -1) {
        // Erase this square.
        state.erased=parseInt(state.erased || 0)+1;
        commitstate(state);
        ...
        // Set the number
        if(pos === hintSuggested){
            state.usefulHintCount=parseInt(state.usefulHintCount || 0)+1;
            commitstate(state);
        }
        ...
        if (victorious(state)) {
            ...
            $(document).trigger('log', ['hint', {
                totalHintCount: currentstate().hintCount,
                usefulHintCount: currentstate().usefulHintCount
            }] );

            $(document).trigger('log', ['GameStats', {
                TotalErasedEvents: parseInt(currentstate().erased || 0),
                TotalFailedSubmits: parseInt(currentstate().failedSubmits || 0),
                TotalClearedInstances: parseInt(currentstate().totalClear || 0),
                TotalRulesClicks: totalRules,
...
$(document).on('click', '#rulesbutton', function (e) {
    var popup = $('#rules').css('display');
    totalRules++;
    if (popup === 'none'){
        showpopup('#rules');
        e.stopPropagation();
...
$(document).on('click', '#hintbutton', function (e) {
    ...
    var prime = [1,2,3,5,7];
    var mul3 = [6,9];
    var hint = SudokuHint.hint(currentstate().puzzle, currentstate().answer, currentstate().work);
    console.log(Sudoku.solvable(boardsofar(state)));
    if(hint == null){
        document.getElementById("hintPopup").innerText = "No hints available.\n Try your luck!";
    }
    else {
        if (hint.hint === 'Conflicts' || hint.hint === 'Mistakes') {
            for (var i = 0; i < hint.errors.length; i++) {
                var tempPosition = hint.errors.pop();
                var position = "#sc" + tempPosition;
                hintSuggested = tempPosition;
                document.getElementById("hintPopup").innerText = hint.hint + " in highlighted positions ";
                $(position).css("background-color", "lightgrey");
            }
        }
        else {
            var binaryNum = hint.exclude.toString(2);
            var pos = [];
            pos.push(binaryNum.length + 1);
            for (var index = binaryNum.length - 1; index >= 0; index--) {
                if (binaryNum[index] === '0') {
                    pos.push(binaryNum.length - index);
                }
            }
            var hintNumb = pos.pop();
            if (prime.indexOf(hintNumb) !== -1) {
                document.getElementById("hintPopup").innerText = "Try figuring out the number in the highlighted position \n Clue: It is a prime number!";
            } else if (mul3.indexOf(hintNumb) !== -1) {
                document.getElementById("hintPopup").innerText = "Try figuring out the number in the highlighted position \n Clue: It is a multiple of 3! ";
            }
            else {
                document.getElementById("hintPopup").innerText = "Try figuring out the number in the highlighted position \n Clue: It is a multiple of 2!";
            }
            var tempPosition1 = hint.reduced.pop();
            var position1 = "#sc" + tempPosition1;
            hintSuggested = tempPosition1;
            $(position1).css("background-color", "darkgrey");
        }
        $(document).click(function () {
            if ($('#hintPopup').css('display') !== 'none') {
                $(document).trigger('log', ['hintDisplay', {
                    HintDisplay: 0
                }])
            }
            $(".sudoku-cell").css("background-color", "white");
        });

        $(document).on('mousedown', 'td.sudoku-cell', function () {
            if ($('#hintPopup').css('display') !== 'none') {
                $(document).trigger('log', ['hintDisplay', {
                    HintDisplay: 0
                }])
        ...
        if ($('#hintPopup').css('display') === 'none') {
            showpopupForHints('#hintPopup');
            state.hintCount=parseInt(state.hintCount || 0)+1;
            commitstate(state);
            e.stopPropagation();
        }
        else {
...
$(document).on('click', '#clearbutton', function (ev) {
    ...
    var cleared = {
        ...
        answer: [], work: [], gentime: (new Date).getTime(), hintCount: state.hintCount,
        usefulHintCount: state.usefulHintCount, erased: state.erased, failedSubmits: state.failedSubmits
    };
    cleared.totalClear=parseInt(state.totalClear || 0)+1;
    commitstate(cleared);
});

// Depressing the "check" button.

$(document).on('mousedown touchstart', '#submitbutton', function (ev) {
    hidepopups();
    var state = currentstate();
    ...
    if (conflicts.length === 0) {
        if(countfilled(sofar) == Sudoku.S)
            showpopup('#victory');
        else {
            state.failedSubmits=parseInt(state.failedSubmits || 0)+1;
        }
    } else {
...
$(document).on('mouseup mouseleave touchend', '#submitbutton', function () {
...
$(document).on('click', '#submitbutton', function (ev) {
    if ($('#victory').css('display') !== 'none') {
        ev.stopPropagation();
    }
});
